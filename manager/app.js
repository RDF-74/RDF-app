...
async function renderReservationPrePlan(reservation) {
  ...
  const plannedStepKeys = new Set(plannedSteps.map((step) => step.step_key));
  const additionalStepMap = new Map();
  vehicleConditionInsights.forEach((item) => {
    if (item.priority.key === "reference") return;
    item.relatedStepKeys.forEach((stepKey) => {
      if (plannedStepKeys.has(stepKey)) return;
      const existing = additionalStepMap.get(stepKey);
      if (!existing) {
        additionalStepMap.set(stepKey, {
          stepKey,
      ...
      if ((plannerPriorityOrder[item.priority.key] ?? 9) < (plannerPriorityOrder[existing.priority.key] ?? 9)) {
        existing.priority = item.priority;
      }
    });
  });
  const additionalSteps = [...additionalStepMap.values()]
    .sort((a, b) => (plannerPriorityOrder[a.priority.key] ?? 9) - (plannerPriorityOrder[b.priority.key] ?? 9)
      || (plannerStepOptions().find((step) => step.step_key === a.stepKey)?.order_group ?? 9999)
      - (plannerStepOptions().find((step) => step.step_key === b.stepKey)?.order_group ?? 9999));
  const additionalStepMarkup = additionalSteps.length
    ? `<section class="card"><h2>追加候補工程</h2><p class="muted">予約コースには入っていない工程のうち、この車の過去履歴から今回確認した方がよいものです。現地確認までは確定しません。</p>${additionalSteps.map((item) => {
        const stepName = plannerStepNameByKey.get(item.stepKey) || item.stepKey;
        const standards = (standardRows || []).filter((standard) => standard.step_key === item.stepKey);
        const standardText = standards.length
          ? standards.map((standard) => {
              const chemical = Array.isArray(standard.recordare_chemicals) ? standard.recordare_chemicals[0] : standard.recordare_chemicals;
              const amount = standard.standard_usage_amount == null ? "" : ` ${standard.standard_usage_amount}${chemical?.unit || "mL"}`;
              return `${recordareChemicalName(chemical)}${amount}`;
            }).join(" / ")
          : "標準ケミカル未設定";
        const reasonText = item.reasons.map((reason) => `${reason.tagKey}${reason.detail ? `（${reason.detail}）` : ""}`).join(" / ");
        return `<div class="service-timing-correction"><div class="planner-priority-row"><p><strong>${escapeHtml(stepName)}</strong></p><span class="planner-priority-badge ${escapeHtml(item.priority.key)}">${escapeHtml(item.priority.label)}</span></div><p class="muted">理由：${escapeHtml(reasonText)}</p><p class="muted">RE:CORDARE標準：${escapeHtml(standardText)}</p></div>`;
      }).join("")}</section>`
    : "";
