...
function renderManager() {
  ...
  document.getElementById("openAnalyticsButton")?.addEventListener("click", () => window.RECORDARE_MANAGER_ANALYTICS?.open("month"));
...
async function renderChemicalPlanner() {
  ...
  const [{ data: chemicals, error: chemicalError }, { data: completedRecords, error: completedError }, { data: usages, error: usageError }, { data: reservations, error: reservationError }, { data: purchases, error: purchaseError }, { data: initialStocks, error: initialStockError }, { data: purchaseOrders, error: purchaseOrderError }, { data: purchaseSources, error: purchaseSourceError }] = await Promise.all([
    ...
    supabase.from("service_records").select("id,course_code").eq("is_active", true).eq("status", "completed"),
...
async function renderCustomerDetail(customerId, returnToReservation = null) {
  ...
  const [{ data: customer, error: customerError }, { data: vehicles, error: vehicleError }, { data: history }, { data: lineLink }] = await Promise.all([
    supabase.from("customers").select("*").eq("id", customerId).maybeSingle(),
    supabase.from("customer_vehicles").select("*").eq("customer_id", customerId).eq("is_active", true).order("created_at"),
    supabase.from("service_records").select("id, vehicle_id, service_date, course_code, selected_options, actual_service_minutes, actual_total_minutes, status").eq("customer_id", customerId).eq("is_active", true).order("service_date", { ascending: false }).limit(30),
    supabase.from("customer_line_links").select("customer_id,linked_at").eq("customer_id", customerId).maybeSingle()
  ]);
  ...
  setCustomerContent(`<div class="card detail-card"><div class="detail-heading"><div><h2>${escapeHtml(customer.name)}</h2><p class="muted">${escapeHtml(contactMethods[customer.contact_method])}</p></div><button class="secondary compact-button" type="button" id="editCustomerButton">編集</button></div><dl><dt>電話番号</dt><dd>${escapeHtml(customer.phone || "未登録")}</dd><dt>LINE表示名</dt><dd>${escapeHtml(customer.line_display_name || "未登録")}</dd><dt>LINE連携</dt><dd>${lineLink?.linked_at ? `連携済み（${escapeHtml(new Date(lineLink.linked_at).toLocaleDateString("ja-JP"))}）` : "未連携"}</dd><dt>備考</dt><dd>${escapeHtml(customer.notes || "未登録")}</dd></dl><button class="text-button danger-text" type="button" id="archiveCustomerButton">この顧客を無効化</button></div><section class="card"><div class="detail-heading"><h2>車両</h2><button class="secondary compact-button" type="button" id="addVehicleButton">＋ 追加</button></div><div class="vehicle-list">${vehicleRows}</div><div id="vehicleFormArea"></div></section><section class="card"><h2>施工履歴</h2><div class="service-history-list">${historyRows}</div></section><button class="text-button" type="button" id="backToCustomers">${returnToReservation ? "← 予約へ戻る" : "← 顧客一覧へ戻る"}</button>`);
...
async function renderVehicleDetail(customerId, vehicleId) {
  ...
  const [{ data: vehicle, error: vehicleError }, { data: history, error: historyError }] = await Promise.all([
    ...
    supabase.from("service_records").select("id, service_date, course_code, selected_options, actual_service_minutes, actual_total_minutes, status").eq("vehicle_id", vehicleId).eq("is_active", true).order("service_date", { ascending: false }).limit(30),
  ]);
...
async function renderReservationList() {
  const { data, error } = await supabase.from("reservations").select("id, customer_id, vehicle_id, course_code, reservation_date, start_time, status, notes, vehicle_size_class, selected_options, selected_discounts, travel_zone, base_price, options_total, travel_fee, discount_total, calculated_total, final_total, planned_prep_minutes, planned_service_minutes, planned_cleanup_minutes, planned_slot_minutes, day_before_line_sent_at, day_before_line_message, customers(name), customer_vehicles(manufacturer, model, color, size_class)").eq("is_active", true).order("reservation_date", { ascending: true }).order("start_time", { ascending: true });
...
async function renderReservationForm(reservation = null) {
  ...
  if (isEdit) {
    ...
    const { data: existingService } = await supabase.from("service_records").select("id").eq("reservation_id", reservation.id).eq("is_active", true).maybeSingle();
...
const serviceOptionSteps = {
  ...
  front_glass_oil_repellent: [
    { step_key: "glass_oil", name: "フロントガラス 油膜除去", order_group: 930 },
    { step_key: "glass_repellent", name: "フロントガラス 撥水", order_group: 1420 },
  ],
  front_glass_scale: [
    { step_key: "glass_scale", name: "フロントガラス ウロコ・油膜除去", order_group: 920 },
    { step_key: "glass_repellent", name: "フロントガラス 撥水", order_group: 1420 },
  ],
  all_glass_oil_repellent: [
    { step_key: "glass_oil", name: "全面ガラス 油膜除去", order_group: 930 },
    { step_key: "glass_repellent", name: "全面ガラス 撥水", order_group: 1420 },
  ],
  all_glass_scale: [
    { step_key: "glass_scale", name: "全面ガラス ウロコ・油膜除去", order_group: 920 },
    { step_key: "glass_repellent", name: "全面ガラス 撥水", order_group: 1420 },
  ],
};
const buildServiceSteps = (courseCode, selectedOptions) => {
  const base = courseCode === "rinseless"
    ...
    : courseCode === "reset_coat" ? serviceStepMaster.filter((step) => step.order_group !== 600)
      : serviceStepMaster.filter((step) => [100, 200, 400, 1100, 1500].includes(step.order_group));
...
const plannerPriorityLabel = (tagKey, details = {}, count = 1, date = "", pastStatus = "重点確認") => {
  const untreated = ["該当工程なし", "スキップ", "未実施", "途中"].includes(pastStatus);
  if (untreated) return { key: "high", label: "優先確認" };
  if (pastStatus === "実施" && count >= 2) return { key: "high", label: "優先確認" };
  const days = date ? Math.floor((Date.now() - new Date(date + "T00:00:00").getTime()) / 86400000) : 0;
  if (pastStatus === "実施" && days > 180) return { key: "reference", label: "参考" };
  if (pastStatus === "実施") return { key: "recommended", label: "確認推奨" };
  const severe = tagKey === "ホイール汚れ強め"
    || tagKey === "鉄粉多め"
    || tagKey === "ガラスウロコあり"
    || (tagKey === "水ジミ・スケール" && ["heavy", "paint_impact"].includes(details.scale_level));
  if (count >= 2 || severe) return { key: "high", label: "優先確認" };
...
const plannerConditionStepKeys = {
  ...
  "ガラス油膜あり": ["glass_oil", "glass_repellent"],
  "ガラスウロコあり": ["glass_scale", "glass_repellent"],
...
async function renderReservationPrePlan(reservation) {
  ...
  (vehicleHistoryRecords || []).forEach((record) => {
    (vehicleConditionsByRecord.get(record.id) || []).forEach((condition) => {
      ...
      const priority = plannerPriorityLabel(condition.tag_key, condition.details || {}, occurrenceCount, record.service_date, pastStatus);
      const stepNames = relatedStepKeys.map((key) => plannerStepNameByKey.get(key) || key);
      const recommendation = !relatedStepKeys.length
        ? "今回も重点確認"
        : pastStatus === "実施"
          ? occurrenceCount >= 2
            ? `処置後に再発履歴あり・${stepNames.join("・")}を優先確認`
            : `前回処置済み・再発していないか${stepNames.join("・")}を確認`
          : pastStatus === "スキップ"
            ? `前回スキップ・${stepNames.join("・")}を優先確認`
            : `前回未処置・${stepNames.join("・")}を優先確認`;
  ...
  const vehicleHistoryMarkup = (vehicleHistoryRecords || []).length
    ? `<section class="card"><h2>この車の過去履歴からの確認候補</h2><p class="muted">この車の完了施工 ${escapeHtml(vehicleHistoryRecords.length)}件を参照しています。過去に状態があって工程を実施した項目は、今回も確認候補として表示します。前回未処置・スキップ、または処置後に同じ症状が再発している場合は「優先確認」。前回処置済みの単発履歴は「確認推奨」、処置済みで180日を超えた単発履歴は「参考」です。</p>${latestCoatingText ? `<p><strong>直近の既存コーティング</strong> ・ ${escapeHtml(latestVehicleRecord.service_date)} ・ ${escapeHtml(latestCoatingText)}</p>` : ""}${vehicleConditionInsights.length ? vehicleConditionInsights.map((item) => `<div class="service-timing-correction"><div class="planner-priority-row"><p><strong>${escapeHtml(item.tagKey)}${item.detail ? `（${escapeHtml(item.detail)}）` : ""}</strong></p><span class="planner-priority-badge ${escapeHtml(item.priority.key)}">${escapeHtml(item.priority.label)}</span></div><p class="muted">${escapeHtml(item.date)} ・ 履歴 ${escapeHtml(item.occurrenceCount)}回 ・ 前回工程：${escapeHtml(item.pastStatus)} ・ ${escapeHtml(item.recommendation)}</p></div>`).join("") : '<p class="muted">施工前状態の履歴はまだありません。</p>'}</section>`
...
const serviceElapsedSeconds = (startedAt, pauses = []) => {
  const startedMs = new Date(startedAt).getTime();
  const nowMs = Date.now();
  const pausedMs = (pauses || []).reduce((total, pause) => {
    if (!pause.started_at) return total;
    const pauseStart = Math.max(startedMs, new Date(pause.started_at).getTime());
    const pauseEnd = Math.min(nowMs, pause.ended_at ? new Date(pause.ended_at).getTime() : nowMs);
    return total + Math.max(0, pauseEnd - pauseStart);
  }, 0);
  return Math.max(0, Math.floor((nowMs - startedMs - pausedMs) / 1000));
};
const formatServiceElapsed = (startedAt, pauses = []) => {
  const seconds = serviceElapsedSeconds(startedAt, pauses);
  const hours = Math.floor(seconds / 3600);
  const minutes = Math.floor((seconds % 3600) / 60);
  return `${String(hours).padStart(2, "0")}:${String(minutes).padStart(2, "0")}:${String(seconds % 60).padStart(2, "0")}`;
};
const showServiceElapsed = (elementId, startedAt, pauses = []) => {
  clearServiceElapsed();
  const update = () => {
    const element = document.getElementById(elementId);
    if (element) element.textContent = formatServiceElapsed(startedAt, pauses);
  };
  update();
...
const serviceConfirmationStatusLabels = {
  unproposed: "未確認",
  adopted: "施工必要",
  declined: "今回は不要",
  proposed: "保留",
};
const serviceCustomerDecisionLabels = {
  not_required: "未実施",
  pending: "未確認",
  approved: "承認",
  declined: "今回は見送り",
...
async function ensureServiceConfirmationProposals(record) {
  if (!record?.vehicle_id) return null;
  const [{ data: existing, error: existingError }, { data: pastRecords, error: recordError }] = await Promise.all([
    supabase.from("service_proposals")
      .select("source_tag_key")
  ...
  if (existingError || recordError) return existingError || recordError;
  if (!(pastRecords || []).length) return null;

  const pastIds = pastRecords.map((item) => item.id);
  const [{ data: conditions, error: conditionError }, { data: steps, error: stepError }] = await Promise.all([
    supabase.from("service_condition_tags")
      .select("service_record_id,tag_key,details")
      .in("service_record_id", pastIds),
    supabase.from("service_steps")
      .select("service_record_id,step_key,started_at,ended_at,skipped_reason")
      .in("service_record_id", pastIds),
  ]);
  if (conditionError || stepError) return conditionError || stepError;

  const existingTags = new Set((existing || []).map((item) => item.source_tag_key));
  const conditionsByRecord = new Map();
  (conditions || []).forEach((item) => {
    if (!conditionsByRecord.has(item.service_record_id)) conditionsByRecord.set(item.service_record_id, []);
    conditionsByRecord.get(item.service_record_id).push(item);
  });
  const stepsByRecord = new Map();
  (steps || []).forEach((item) => {
    if (!stepsByRecord.has(item.service_record_id)) stepsByRecord.set(item.service_record_id, []);
    stepsByRecord.get(item.service_record_id).push(item);
  });
  const countByTag = new Map();
  (conditions || []).forEach((item) => countByTag.set(item.tag_key, (countByTag.get(item.tag_key) || 0) + 1));

  const seen = new Set();
  const values = [];
  (pastRecords || []).forEach((pastRecord) => {
    (conditionsByRecord.get(pastRecord.id) || []).forEach((condition) => {
      if (seen.has(condition.tag_key) || existingTags.has(condition.tag_key)) return;
      seen.add(condition.tag_key);
      const stepKeys = plannerConditionStepKeys[condition.tag_key] || [];
      const relatedSteps = (stepsByRecord.get(pastRecord.id) || []).filter((step) => stepKeys.includes(step.step_key));
      const pastStatus = stepKeys.length ? plannerPastStepStatus(relatedSteps) : "重点確認";
      const occurrenceCount = countByTag.get(condition.tag_key) || 1;
      const priority = plannerPriorityLabel(condition.tag_key, condition.details || {}, occurrenceCount, pastRecord.service_date, pastStatus);
      values.push({
        service_record_id: record.id,
        title: condition.tag_key,
        priority: priority.key,
        status: "unproposed",
        source_tag_key: condition.tag_key,
        step_keys: stepKeys,
        source_service_record_id: pastRecord.id,
        snapshot: {
          source_date: pastRecord.service_date,
          source_details: condition.details || {},
          detail: plannerConditionDetail(condition.tag_key, condition.details || {}),
          occurrence_count: occurrenceCount,
          past_status: pastStatus,
  ...
  const { error } = await supabase.from("service_proposals").insert(values);
  return error || null;
}
...
const serviceConfirmationMarkup = (proposals, steps, interactive = true) => {
  ...
  const instructionText = interactive
    ? "現車を確認して結果を選びます。追加施工が必要な項目は、次のお客様確認で了承後に工程へ反映します。"
  ...
  return `<section class="card"><h2>過去履歴の確認候補</h2><p class="muted">${escapeHtml(instructionText)}</p>${sorted.map((proposal) => {
    ...
    const targetText = !keys.length
      ? "対応工程は未設定"
      : alreadyIncluded
        ? `対象工程：${targetNames.join("・")}（今回の工程に含まれています）`
        : `対象工程：${targetNames.join("・")}（施工必要なら追加）`;
...
const bindServiceConfirmationResults = (recordId, rerender) => {
  document.querySelectorAll("[data-confirmation-result]").forEach((button) => button.addEventListener("click", async () => {
    const buttons = document.querySelectorAll(`[data-proposal-id="${button.dataset.proposalId}"]`);
    buttons.forEach((item) => { item.disabled = true; });
    const { error } = await supabase.from("service_proposals")
      .update({ status: button.dataset.confirmationResult, customer_decision: "not_required", customer_confirmed_at: null })
      .eq("id", button.dataset.proposalId)
...
async function applyServiceConfirmationResults(recordId, proposals, steps) {
  const requiredPending = (proposals || []).filter((proposal) =>
    ["high", "recommended"].includes(proposal.priority) && ["unproposed", "proposed"].includes(proposal.status)
  );
  if (requiredPending.length) {
    return { error: new Error("確認候補の「優先確認・確認推奨」は、施工必要または今回は不要を確定してから次へ進んでください。"), needsCustomerConfirmation: false };
  }

  const adopted = (proposals || []).filter((proposal) => proposal.status === "adopted");
  if (!adopted.length) return { error: null, needsCustomerConfirmation: false };

  const conditionValues = adopted
    .filter((proposal) => proposal.source_tag_key)
    .map((proposal) => ({
      service_record_id: recordId,
      tag_key: proposal.source_tag_key,
      details: proposal.snapshot?.source_details || {},
    }));
  if (conditionValues.length) {
    const { error: conditionError } = await supabase.from("service_condition_tags")
      .upsert(conditionValues, { onConflict: "service_record_id,tag_key" });
    if (conditionError) return { error: conditionError, needsCustomerConfirmation: false };
  }

  const needsCustomerConfirmation = adopted.filter((proposal) => serviceConfirmationMissingStepKeys(proposal, steps).length);
  if (!needsCustomerConfirmation.length) return { error: null, needsCustomerConfirmation: false };

  const { error } = await supabase.from("service_proposals")
    .update({ customer_decision: "pending", customer_confirmed_at: null })
    .in("id", needsCustomerConfirmation.map((proposal) => proposal.id))
    .eq("service_record_id", recordId);
  return { error: error || null, needsCustomerConfirmation: !error };
}

async function addApprovedServiceConfirmationSteps(recordId, proposals) {
  const approved = (proposals || []).filter((proposal) => proposal.status === "adopted" && proposal.customer_decision === "approved");
  if (!approved.length) return null;

  const stepByKey = new Map(plannerStepOptions().map((step) => [step.step_key, step]));
  const addedKeys = new Set();
  const stepValues = [];
  approved.forEach((proposal) => jsonArray(proposal.step_keys).forEach((stepKey) => {
    if (addedKeys.has(stepKey)) return;
    const step = stepByKey.get(stepKey);
    if (!step) return;
    addedKeys.add(stepKey);
    stepValues.push({
      step_key: step.step_key,
      step_name: step.name,
      order_group: step.order_group,
      timed: step.timed !== false,
      skippable: step.skippable !== false,
      snapshot: {
        ...step,
        source: "customer_confirmation",
        source_tag_key: proposal.source_tag_key,
      },
    });
  }));
  if (!stepValues.length) return null;
  const { error } = await supabase.rpc("add_service_confirmation_steps", {
...
async function renderServiceCustomerConfirmation(recordId, record, activeStep, proposals, steps) {
  clearServiceElapsed();
  const stageProposals = (proposals || []).filter((proposal) =>
    proposal.status === "adopted" && proposal.customer_decision !== "not_required"
  );
  const stepNames = new Map(plannerStepOptions().map((step) => [step.step_key, step.name]));
  ...
  const rows = stageProposals.map((proposal) => {
    const snapshot = proposal.snapshot || {};
    const detail = snapshot.detail ? `（${snapshot.detail}）` : "";
    const missingNames = jsonArray(proposal.step_keys)
      .filter((stepKey) => !existingKeys.has(stepKey))
      .map((stepKey) => stepNames.get(stepKey) || stepKey);
    const status = serviceCustomerDecisionLabels[proposal.customer_decision] || "未確認";
    return `<div class="service-timing-correction"><p><strong>${escapeHtml(proposal.source_tag_key || proposal.title)}${escapeHtml(detail)}</strong></p><p class="muted">追加施工：${escapeHtml(missingNames.join("・") || "追加工程なし")}</p><p class="muted">お客様確認：${escapeHtml(status)}</p><div class="service-confirmation-actions"><button type="button" class="filter-button ${proposal.customer_decision === "approved" ? "active" : ""}" data-customer-confirmation="approved" data-proposal-id="${escapeHtml(proposal.id)}">承認</button><button type="button" class="filter-button ${proposal.customer_decision === "declined" ? "active" : ""}" data-customer-confirmation="declined" data-proposal-id="${escapeHtml(proposal.id)}">今回は見送る</button></div></div>`;
  }).join("");

  setServiceContent(`<div class="card detail-card"><div class="detail-heading"><div><h2>${escapeHtml(record.customer_name)}</h2><p class="muted">${escapeHtml(`${record.vehicle_manufacturer} ${record.vehicle_model}`)}</p></div><span class="reservation-status">お客様確認</span></div><h2>追加施工の確認</h2><p class="muted">施工前確認で必要と判断した追加施工です。お客様の了承を確認してから工程へ反映します。</p></div><section class="card">${rows}</section><button class="primary service-action-button" type="button" id="confirmCustomerServiceButton">お客様確認を確定して施工へ進む</button><button class="text-button" type="button" id="backToPreCheckButton">← 施工前確認に戻る</button>${activeServiceDeleteMarkup}<button class="text-button" type="button" id="backToServiceList">← 施工一覧へ戻る</button>`);

  document.querySelectorAll("[data-customer-confirmation]").forEach((button) => button.addEventListener("click", async () => {
    const related = document.querySelectorAll(`[data-proposal-id="${button.dataset.proposalId}"]`);
    related.forEach((item) => { item.disabled = true; });
    const decision = button.dataset.customerConfirmation;
    const { error } = await supabase.from("service_proposals")
      .update({ customer_decision: decision, customer_confirmed_at: new Date().toISOString() })
      .eq("id", button.dataset.proposalId)
      .eq("service_record_id", recordId);
    if (error) {
  ...
  document.getElementById("backToPreCheckButton").addEventListener("click", async () => {
    const ids = stageProposals.map((proposal) => proposal.id);
    if (ids.length) {
      const { error } = await supabase.from("service_proposals")
        .update({ customer_decision: "not_required", customer_confirmed_at: null })
        .in("id", ids)
        .eq("service_record_id", recordId);
      if (error) return alert(saveErrorMessage(error));
    }
  ...
  document.getElementById("confirmCustomerServiceButton").addEventListener("click", async (event) => {
    const pending = stageProposals.filter((proposal) => proposal.customer_decision === "pending");
    if (pending.length) return alert("追加施工は、お客様の承認または今回は見送るを選んでください。");
    const button = event.currentTarget;
    button.disabled = true;
    button.textContent = "反映中…";

    const addError = await addApprovedServiceConfirmationSteps(recordId, stageProposals);
    if (addError) {
      button.disabled = false;
      button.textContent = "お客様確認を確定して施工へ進む";
      return alert(saveErrorMessage(addError));
    }

    const { data: freshSteps, error: stepError } = await supabase.from("service_steps")
      .select("id,started_at,ended_at,sequence_no")
      .eq("service_record_id", recordId)
    ...
    const now = new Date().toISOString();
    const { error: endError } = await supabase.from("service_steps")
      .update({ ended_at: now })
      .eq("id", activeStep.id)
      .is("ended_at", null);
    if (endError) return alert(saveErrorMessage(endError));

    const currentIndex = (freshSteps || []).findIndex((step) => step.id === activeStep.id);
    const nextStep = currentIndex >= 0 ? freshSteps[currentIndex + 1] : null;
    if (nextStep) {
      const { error: startError } = await supabase.from("service_steps")
...
const recentDeletedServiceCutoff = () => new Date(Date.now() - 30 * 86400000).toISOString();

async function renderRecentlyDeletedServices() {
  clearServiceElapsed();
  const token = ++serviceViewToken;
  setServiceContent('<div class="card placeholder"><p class="muted">最近削除した項目を読み込んでいます…</p></div>');
  const { data, error } = await supabase.from("service_records")
    .select("id,reservation_id,customer_name,vehicle_manufacturer,vehicle_model,course_code,service_date,status,deleted_at")
    .eq("is_active", false)
    .not("deleted_at", "is", null)
    .gte("deleted_at", recentDeletedServiceCutoff())
    .order("deleted_at", { ascending: false });
  if (token !== serviceViewToken || activeTab !== "施工") return;
  if (error) return setServiceContent('<div class="card"><p class="error">最近削除した項目を読み込めませんでした。</p><button class="text-button" id="backToServiceList">← 施工一覧へ戻る</button></div>');

  const rows = (data || []).length
    ? data.map((record) => {
        const deletedLabel = record.deleted_at ? new Date(record.deleted_at).toLocaleString("ja-JP", { month: "numeric", day: "numeric", hour: "2-digit", minute: "2-digit" }) : "";
        return `<div class="card detail-card"><div class="detail-heading"><div><h2>${escapeHtml(record.customer_name)}</h2><p class="muted">${escapeHtml(`${record.vehicle_manufacturer} ${record.vehicle_model}`)}</p></div><span class="reservation-status">削除済み</span></div><dl><dt>施工日</dt><dd>${escapeHtml(reservationDate(record.service_date))}</dd><dt>コース</dt><dd>${escapeHtml(reservationCourses[record.course_code] || record.course_code)}</dd><dt>削除日時</dt><dd>${escapeHtml(deletedLabel)}</dd></dl><button class="secondary" type="button" data-restore-service-record="${escapeHtml(record.id)}" data-reservation-id="${escapeHtml(record.reservation_id)}">施工履歴を復元</button></div>`;
      }).join("")
    : '<div class="empty-state">30日以内に削除した施工履歴はありません。</div>';

  setServiceContent(`<section class="card"><h2>最近削除した項目</h2><p class="muted">削除から30日以内の施工履歴を表示しています。工程・施工前状態・使用ケミカルなどの紐づく記録も一緒に復元されます。</p></section>${rows}<button class="text-button" type="button" id="backToServiceList">← 施工一覧へ戻る</button>`);

  document.querySelectorAll("[data-restore-service-record]").forEach((button) => button.addEventListener("click", async () => {
    const recordId = button.dataset.restoreServiceRecord;
    const reservationId = button.dataset.reservationId;
    const { data: activeRecord, error: activeError } = await supabase.from("service_records")
      .select("id")
      .eq("reservation_id", reservationId)
      .eq("is_active", true)
      .maybeSingle();
    if (activeError) return alert(saveErrorMessage(activeError));
    if (activeRecord?.id) return alert("同じ予約に新しい施工記録があるため、この履歴は復元できません。");
    if (!confirm("この施工履歴を復元しますか？")) return;
    button.disabled = true;
    button.textContent = "復元中…";
    const { error: restoreError } = await supabase.from("service_records")
      .update({ is_active: true, deleted_at: null })
      .eq("id", recordId)
      .eq("is_active", false);
    if (restoreError) {
      button.disabled = false;
      button.textContent = "施工履歴を復元";
      return alert(saveErrorMessage(restoreError));
    }
    await renderRecentlyDeletedServices();
  }));
...
async function startServiceTimer(recordId, button) {
  ...
  const { data: record, error: recordError } = await supabase.from("service_records").select("course_code, selected_options").eq("id", recordId).eq("is_active", true).maybeSingle();
...
const serviceChemicalUsageMarkup = (activeStep, usages, chemicals, standards = []) => {
  ...
  const stepStandards = (standards || []).filter((item) => item.step_key === activeStep.step_key && item.recordare_chemical_id);
  const standardByChemicalId = new Map(stepStandards.map((item) => [item.recordare_chemical_id, item]));
  ...
  const standardGuideMarkup = stepStandards.length
    ? `<div class="service-timing-correction"><h3>RE:CORDARE標準</h3>${stepStandards.map((standard) => {
        const chemical = chemicalById.get(standard.recordare_chemical_id);
        if (!chemical) return "";
        const amountText = standard.standard_usage_amount == null
          ? "使用量目安 未設定"
          : `使用量目安 ${standard.standard_usage_amount}${chemical.unit || "mL"}`;
        return `<p><strong>${escapeHtml(recordareChemicalName(chemical))}</strong></p><p class="muted">${escapeHtml(amountText)}${standard.notes ? ` ・ ${escapeHtml(standard.notes)}` : ""}</p>`;
      }).join("")}</div>`
    : "";

  const candidates = (chemicals || [])
    .filter((chemical) => chemical.status === "active" && !linkedIds.has(chemical.id))
    .sort((a, b) => Number(!standardByChemicalId.has(a.id)) - Number(!standardByChemicalId.has(b.id)));
  const firstStandardCandidate = candidates.find((chemical) => standardByChemicalId.has(chemical.id));
  const candidateOptions = candidates.map((chemical) => {
    const stock = chemical.current_stock == null ? "在庫未登録" : `${chemical.current_stock}${chemical.unit || "mL"}`;
    const standard = standardByChemicalId.get(chemical.id);
    const standardAmount = standard?.standard_usage_amount;
    const standardSuffix = standard
      ? standardAmount == null
        ? " ・ 標準"
        : ` ・ 標準 ${standardAmount}${chemical.unit || "mL"}`
      : "";
    const selected = firstStandardCandidate?.id === chemical.id ? " selected" : "";
    const standardData = standardAmount == null
      ? ""
      : ` data-standard-amount="${escapeHtml(standardAmount)}" data-standard-unit="${escapeHtml(chemical.unit || "mL")}"`;
    return `<option value="${escapeHtml(chemical.id)}"${selected}${standardData}>${escapeHtml(recordareChemicalName(chemical))}（${escapeHtml(stock)}${escapeHtml(standardSuffix)}）</option>`;
  }).join("");
  const firstStandard = firstStandardCandidate ? standardByChemicalId.get(firstStandardCandidate.id) : null;
  const firstStandardPlaceholder = firstStandard?.standard_usage_amount == null
    ? ""
    : `標準 ${firstStandard.standard_usage_amount}${firstStandardCandidate?.unit || "mL"}`;
  const addForm = candidateOptions
    ? `<form class="service-timing-correction" data-service-chemical-form>
        ...
        <label>実使用量mL<input name="actual_amount" type="number" inputmode="decimal" min="0.001" step="0.001" placeholder="${escapeHtml(firstStandardPlaceholder)}" /></label>
        <label>メモ<input name="notes" /></label>
        <button class="secondary" type="submit">記録</button>
      </form>`
    : '<p class="muted">追加できるマイケミカルはありません。</p>';
  return `<section class="card"><h2>使用ケミカル</h2><p class="muted">この工程で実際に使用したケミカルを記録します。標準使用量は目安なので、保存するのは実際に使った量です。</p>${standardGuideMarkup}${usageRows || '<p class="muted">まだ記録されていません。</p>'}${addForm}</section>`;
};
...
const bindServiceChemicalUsageForms = (recordId, activeStep) => {
  document.querySelectorAll("[data-service-chemical-form]").forEach((form) => {
    ...
    const chemicalSelect = form.elements.chemical_id;
    const syncStandardHint = () => {
      const option = chemicalSelect?.selectedOptions?.[0];
      const standardAmount = option?.dataset?.standardAmount;
      const standardUnit = option?.dataset?.standardUnit || "mL";
      amount.placeholder = standardAmount ? `標準 ${standardAmount}${standardUnit}` : "";
    };
    const syncAmount = () => {
    ...
    syncStandardHint();
    chemicalSelect?.addEventListener?.("change", syncStandardHint);
    syncAmount();
...
async function renderServiceTimer(recordId) {
  ...
  const [{ data: record, error: recordError }, { data: steps, error: stepsError }, { data: savedConditions }, { data: sessions, error: sessionsError }, { data: pauses }, { data: chemicalUsages, error: chemicalUsagesError }, { data: chemicals, error: chemicalsError }, { data: chemicalStandards, error: chemicalStandardsError }] = await Promise.all([
    ...
    supabase.from("service_pauses").select("id, started_at, ended_at").eq("service_record_id", recordId),
    ...
    supabase.from("recordare_chemical_step_standards").select("recordare_chemical_id,step_key,standard_usage_amount,notes").eq("is_active", true).eq("course_code", "all"),
  ]);
  if (token !== serviceViewToken || activeTab !== "施工") return;
  if (recordError || stepsError || sessionsError || chemicalUsagesError || chemicalsError || chemicalStandardsError || !record) return setServiceContent('<div class="card"><p class="error">工程を読み込めませんでした。</p></div>');
  ...
  if (activePause) {
    ...
    document.getElementById("resumeServiceButton").addEventListener("click", async (event) => {
      const button = event.currentTarget;
      button.disabled = true;
      button.textContent = "再開中…";
      const { error } = await supabase.from("service_pauses").update({ ended_at: new Date().toISOString() }).eq("id", activePause.id).is("ended_at", null);
      if (error) {
        button.disabled = false;
        button.textContent = "施工を再開";
        return alert(saveErrorMessage(error));
      }
      await renderServiceTimer(recordId);
    });
  ...
  if (isPreCheck) {
    const confirmationError = await ensureServiceConfirmationProposals(record);
    if (confirmationError) {
      return setServiceContent(`<div class="card"><p class="error">${escapeHtml(saveErrorMessage(confirmationError))}</p></div>`);
    }
    const { data: proposalRows, error: proposalError } = await supabase.from("service_proposals")
      .select("id,title,priority,status,source_tag_key,step_keys,source_service_record_id,snapshot,customer_decision,customer_confirmed_at")
      .eq("service_record_id", recordId)
      .not("source_tag_key", "is", null)
      .order("created_at", { ascending: true });
    if (proposalError) {
      return setServiceContent(`<div class="card"><p class="error">${escapeHtml(saveErrorMessage(proposalError))}</p></div>`);
    }
    confirmationProposals = proposalRows || [];
    const customerConfirmationStage = confirmationProposals.some((proposal) =>
      proposal.status === "adopted" && proposal.customer_decision !== "not_required"
    );
    if (customerConfirmationStage) {
      return renderServiceCustomerConfirmation(recordId, record, activeStep, confirmationProposals, steps);
    }
  ...
  document.getElementById("pauseServiceButton")?.addEventListener("click", async (event) => {
    const button = event.currentTarget;
    button.disabled = true;
    button.textContent = "停止中…";
    const { error } = await supabase.from("service_pauses").insert({
      service_record_id: recordId,
  ...
  document.getElementById("previousServiceStepButton")?.addEventListener("click", async (event) => {
    if (!previousStep) return;
    const button = event.currentTarget;
    button.disabled = true;
    button.textContent = "戻っています…";
    const originalStartedAt = activeStep.started_at;
    const { error: currentError } = await supabase.from("service_steps")
      .update({ started_at: null })
      .eq("id", activeStep.id)
      .is("ended_at", null);
    if (currentError) {
      button.disabled = false;
      button.textContent = "← 前の工程へ戻る";
      return alert(saveErrorMessage(currentError));
    }
    const { error: previousError } = await supabase.from("service_steps")
      .update({ ended_at: null, skipped_reason: null })
      .eq("id", previousStep.id);
    if (previousError) {
      await supabase.from("service_steps")
        .update({ started_at: originalStartedAt })
        .eq("id", activeStep.id)
        .is("started_at", null);
      button.disabled = false;
      button.textContent = "← 前の工程へ戻る";
      return alert(saveErrorMessage(previousError));
    }
    await renderServiceTimer(recordId);
  });
  ...
  document.getElementById("nextServiceStepButton").addEventListener("click", async (event) => {
    ...
    button.disabled = true;
    if (isPreCheck) {
      const confirmationApply = await applyServiceConfirmationResults(recordId, confirmationProposals, steps);
      if (confirmationApply.error) {
        button.disabled = false;
        return alert(saveErrorMessage(confirmationApply.error));
      }
      if (confirmationApply.needsCustomerConfirmation) {
        return renderServiceTimer(recordId);
      }
    }
    const now = new Date().toISOString();
...
async function renderServiceDetail(recordId) {
  ...
  const { data: record, error } = await supabase.from("service_records").select("*").eq("id", recordId).eq("is_active", true).maybeSingle();
  if (token !== serviceViewToken || activeTab !== "施工") return;
  ...
  if (record.status === "completed") {
    const { data: confirmationRows, error: confirmationHistoryError } = await supabase.from("service_proposals")
      .select("title,priority,status,source_tag_key,step_keys,snapshot,customer_decision,customer_confirmed_at")
      .eq("service_record_id", recordId)
      .not("source_tag_key", "is", null)
      .order("created_at", { ascending: true });
    if (confirmationHistoryError) {
      confirmationHistoryMarkup = '<section class="card"><h2>確認候補の結果</h2><p class="error">確認結果を読み込めませんでした。</p></section>';
    } else if ((confirmationRows || []).length) {
      const priorityLabels = { high: "優先確認", recommended: "確認推奨", reference: "参考" };
      confirmationHistoryMarkup = `<section class="card"><h2>確認候補の結果</h2>${confirmationRows.map((item) => {
        const snapshot = item.snapshot || {};
        const detail = snapshot.detail ? `（${snapshot.detail}）` : "";
        const customerDecision = item.customer_decision && item.customer_decision !== "not_required"
          ? ` ・ お客様確認：${serviceCustomerDecisionLabels[item.customer_decision] || item.customer_decision}`
          : "";
        return `<div class="service-timing-correction"><div class="planner-priority-row"><p><strong>${escapeHtml(item.source_tag_key || item.title)}${escapeHtml(detail)}</strong></p><span class="planner-priority-badge ${escapeHtml(item.priority || "reference")}">${escapeHtml(priorityLabels[item.priority] || "参考")}</span></div><p class="muted">確認結果：${escapeHtml(serviceConfirmationStatusLabels[item.status] || item.status)}${escapeHtml(customerDecision)}</p></div>`;
      }).join("")}</section>`;
    }
  }
  ...
  document.getElementById("deleteServiceRecordButton")?.addEventListener("click", async () => {
    if (!confirm("この施工履歴を削除しますか？30日間は「最近削除した項目」から復元できます。")) return;
    const button = document.getElementById("deleteServiceRecordButton");
    button.disabled = true;
    button.textContent = "削除中…";
    const { error } = await supabase.from("service_records")
      .update({ is_active: false, deleted_at: new Date().toISOString() })
...
async function createServiceRecordFromReservation(reservationId, button) {
  try {
    const { data: existing, error: existingError } = await supabase.from("service_records").select("id").eq("reservation_id", reservationId).eq("is_active", true).maybeSingle();
