...
async function renderChemicalList() {
  ...
  const { data, error } = await supabase.from("recordare_chemicals").select("id,status,unit,current_stock,chemical_catalog_products(manufacturer,product_name,category)").order("created_at", { ascending: false });
  if (error) return setCustomerContent(`<div class="card"><p class="error">${escapeHtml(saveErrorMessage(error))}</p></div>`);
  const rows = data.length ? data.map((item) => `<button class="customer-row" type="button" data-chemical-id="${item.id}"><span><strong>${escapeHtml(item.chemical_catalog_products.manufacturer)} ${escapeHtml(item.chemical_catalog_products.product_name)}</strong><small>${escapeHtml(item.chemical_catalog_products.category || "未分類")} ・ 在庫 ${item.current_stock == null ? "未登録" : `${escapeHtml(item.current_stock)}${escapeHtml(item.unit)}`} ・ ${item.status === "active" ? "使用中" : "休止"}</small></span><span>›</span></button>`).join("") : '<p class="muted">マイケミカルはまだありません。</p>';
  setCustomerContent(`<button class="primary" type="button" id="addChemicalButton">＋ ケミカルを追加</button><div class="customer-list">${rows}</div><button class="text-button" type="button" id="backHome">← ホームへ戻る</button>`);
...
async function renderChemicalDetail(id) {
  ...
  setCustomerContent(`<div class="card detail-card"><h2>${escapeHtml(c.chemical_catalog_products.manufacturer)} ${escapeHtml(c.chemical_catalog_products.product_name)}</h2><dl><dt>現在在庫</dt><dd>${stockText}</dd><dt>状態</dt><dd>${c.status==="active"?"使用中":"休止"}</dd></dl><button class="secondary" id="purchaseChemical">購入・補充</button><button class="secondary" id="initialChemical">初期在庫を登録</button><button class="secondary" id="adjustChemical">在庫を修正</button></div><section class="card"><h2>購入履歴</h2>${(purchases||[]).map(x=>`<p>${x.purchased_on} ・ ${x.capacity}mL × ${x.quantity}本${x.amount==null?" ・ 価格未設定":` ・ ¥${x.amount}`}</p>`).join("")||"<p class=muted>なし</p>"}</section><section class="card"><h2>在庫修正履歴</h2>${(adjustments||[]).map(x=>`<p>${x.adjusted_on} ・ ${x.reason} ・ ${x.previous_stock == null ? "未登録" : x.previous_stock}→${x.new_stock}${escapeHtml(c.unit)}</p>`).join("")||"<p class=muted>なし</p>"}</section><button class="text-button" id="backChemicals">← 一覧へ戻る</button>`);
