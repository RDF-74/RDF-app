...
const bindServiceConfirmationResults = (recordId, rerender) => {
  document.querySelectorAll("[data-confirmation-result]").forEach((button) => button.addEventListener("click", async () => {
    ...
    const { error } = await supabase.from("service_proposals")
      .update({ status: button.dataset.confirmationResult })
...
async function applyServiceConfirmationResults(recordId, proposals) {
  ...
  if (requiredPending.length) {
    return new Error("確認候補の「優先確認・確認推奨」は、施工必要または今回は不要を確定してから次へ進んでください。");
  }
  ...
  const stepValues = [];
  adopted.forEach((proposal) => jsonArray(proposal.step_keys).forEach((stepKey) => {
...
async function renderServiceTimer(recordId) {
  ...
  document.getElementById("nextServiceStepButton").addEventListener("click", async (event) => {
    ...
    button.disabled = true;
    if (isPreCheck) {
      const confirmationApplyError = await applyServiceConfirmationResults(recordId, confirmationProposals);
      if (confirmationApplyError) {
        button.disabled = false;
        return alert(saveErrorMessage(confirmationApplyError));
      }
    }
    const now = new Date().toISOString();
...
async function renderServiceDetail(recordId) {
  ...
  if (record.status === "completed") {
    const { data: confirmationRows, error: confirmationHistoryError } = await supabase.from("service_proposals")
      .select("title,priority,status,source_tag_key,step_keys,snapshot")
      .eq("service_record_id", recordId)
      .not("source_tag_key", "is", null)
      .order("created_at", { ascending: true });
    if (confirmationHistoryError) {
      ...
      confirmationHistoryMarkup = `<section class="card"><h2>確認候補の結果</h2>${confirmationRows.map((item) => {
        ...
        const detail = snapshot.detail ? `（${snapshot.detail}）` : "";
        return `<div class="service-timing-correction"><div class="planner-priority-row"><p><strong>${escapeHtml(item.source_tag_key || item.title)}${escapeHtml(detail)}</strong></p><span class="planner-priority-badge ${escapeHtml(item.priority || "reference")}">${escapeHtml(priorityLabels[item.priority] || "参考")}</span></div><p class="muted">確認結果：${escapeHtml(serviceConfirmationStatusLabels[item.status] || item.status)}</p></div>`;
      }).join("")}</section>`;
    }
  }
